...
png(filename="OLA_LESS_INT.png", width=900, bg="white")
...
```{r}
png(filename="OLA_MORE_INT.png", width=900, bg="white")
par(mar=c(2,4,4,10)+.1)
heatmap.2(
...
p <- ggplot(df, aes(x = lfcs, y = -log10(pvals), colour = significant, gene = gene)) +
      geom_point(alpha = 0.8, size = 2) +
      labs(title = 'Volcano Plot, Olaparib, More Resistant vs Intermediate',x = 'log2 fold change', y = '-log10 p-value') +
      scale_color_manual(values = c('#619CFF', '#F564E3'))
      p
```
...
write.csv(res.table, file = paste0('DESeq2_results-ola-res_more_vs_int.csv'))
```
...
write.csv(res.table, file = paste0('DESeq2_results-ola-res_less_vs_int.csv'))
